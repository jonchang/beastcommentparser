...
class BeastCommentParser:
    ...
    def __init__(self, string):
        """Returns a `BeastCommentParser` object when passed a comment
    ...
    def parse(self):
        """Parses the comment string associated with the current object.
        Returns a dict if successful, or raises an Exception otherwise.
        ...
        Traceback (most recent call last):
            ...
        Exception: Expected '&' but got 'height'
        """
        try:
    ...
    def _parse(self, stream):
        ...
        while self.token is not None:
            identifier = self._expect('IDENTIFIER',
                self._not_special).strip('"')
            self._expect('=')
            if (self._not_special(self._advance())):
                self.parsed[identifier] = self._check('VALUE',
                    self._not_special).strip('"')
                self._expect(',')
            else:
                array = []
                ...
                while self.token is not '}':
                    array.append(self._expect('VALUE',
                        self._not_special).strip('"'))
                    self._expect(',}')
    ...
    def _check(self, wanted, test=None):
        """Checks that the current token is what we want, otherwise raise
            an Exception.
        """
        if test is None:
        ...
        if not test:
            raise Exception("Expected %s but got %s" % (wanted, self.token))
        return self.token
